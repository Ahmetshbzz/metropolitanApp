...
import type { BusMode, PublishOptions } from './types';
...
// Helper publishers
export async function publishEphemeral<T>(name: string, payload: T, options?: Omit<PublishOptions, 'durability'>): Promise<void> {
  await eventBus.publish<T>(name, payload, { ...options, durability: 'ephemeral', broadcast: true });
}

export async function publishPersistent<T>(name: string, payload: T, options?: Omit<PublishOptions, 'durability'>): Promise<void> {
