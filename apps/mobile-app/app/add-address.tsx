...
export default function AddAddressScreen() {
  ...
  const createAddress = async (addressData: any) => {
    console.log("Mock: Creating address", addressData);
  ...
  const setDeliveryAddress = (address: any) => {
    console.log("Mock: Setting delivery address", address);
  };
  ...
  const handleAddAddress = async () => {
    ...
    try {
      const result = await createAddress({
        ...formData,
        // userId is set automatically on the backend
      ...
      // If we're in checkout flow and address was created successfully, set it as delivery address
      if (result?.success && result?.address) {
        // Check if we came from checkout by looking at the previous route
        const routes = router.canGoBack() ? navigation.getState()?.routes : [];
        const cameFromCheckout = routes?.some((route: any) => 
