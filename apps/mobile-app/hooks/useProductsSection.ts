...
import type { OrderItem } from "@metropolitan/shared";
...
export const useProductsSection = (items: OrderItem[]) => {
  ...
  const addAvailableStock = async (stockChecks: any[]) => {
    try {
      ...
      for (const item of availableItems) {
        ...
        try {
          // Mock add to cart
          console.log(`Mock adding ${quantityToAdd} of ${item.product.name}`);
        } catch (error) {
  ...
  const addAllItemsToCart = async (items: OrderItem[]) => {
    try {
      ...
      console.log('Mock adding all items to cart:', items.length);
